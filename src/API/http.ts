...
export class NavigationAPI {
  ...
  async initDB(): Promise<{ success: boolean; alreadyInitialized: boolean }> {
    ...
    // 先创建groups表
    await this.db.exec(
      `CREATE TABLE IF NOT EXISTS groups (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, order_num INTEGER NOT NULL, hidden INTEGER NOT NULL DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP);`
    );

    // 再创建sites表
    await this.db.exec(
      `CREATE TABLE IF NOT EXISTS sites (id INTEGER PRIMARY KEY AUTOINCREMENT, group_id INTEGER NOT NULL, name TEXT NOT NULL, url TEXT NOT NULL, icon TEXT, description TEXT, notes TEXT, order_num INTEGER NOT NULL, hidden INTEGER NOT NULL DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (group_id) REFERENCES groups(id) ON DELETE CASCADE);`
    );

    // 兼容已有数据库，尝试添加缺失的列（如果已存在则忽略错误）
    try {
      await this.db.exec('ALTER TABLE groups ADD COLUMN hidden INTEGER NOT NULL DEFAULT 0');
    } catch {}
    try {
      await this.db.exec('ALTER TABLE sites ADD COLUMN hidden INTEGER NOT NULL DEFAULT 0');
    } catch {}
  ...
  async getGroups(): Promise<Group[]> {
    const result = await this.db
      .prepare('SELECT id, name, order_num, hidden, created_at, updated_at FROM groups ORDER BY order_num')
      .all<Group>();
  ...
  async getGroup(id: number): Promise<Group | null> {
    const result = await this.db
      .prepare('SELECT id, name, order_num, hidden, created_at, updated_at FROM groups WHERE id = ?')
      .bind(id)
      .first<Group>();
    return result;
  }
  ...
  async createGroup(group: Group): Promise<Group> {
    const result = await this.db
      .prepare(
        'INSERT INTO groups (name, order_num, hidden) VALUES (?, ?, ?) RETURNING id, name, order_num, hidden, created_at, updated_at'
      )
      .bind(group.name, group.order_num, group.hidden ? 1 : 0)
      .all<Group>();
  ...
  async updateGroup(id: number, group: Partial<Group>): Promise<Group | null> {
    ...
    if (group.hidden !== undefined) {
      updates.push('hidden = ?');
      params.push(group.hidden ? 1 : 0);
    }

    // 构建安全的参数化查询
    const query = `UPDATE groups SET ${updates.join(
      ', '
    )} WHERE id = ? RETURNING id, name, order_num, hidden, created_at, updated_at`;
    params.push(id);
  ...
  async getSites(groupId?: number): Promise<Site[]> {
    let query =
      'SELECT id, group_id, name, url, icon, description, notes, order_num, hidden, created_at, updated_at FROM sites';
    const params: (string | number)[] = [];
  ...
  async getSite(id: number): Promise<Site | null> {
    const result = await this.db
      .prepare(
        'SELECT id, group_id, name, url, icon, description, notes, order_num, hidden, created_at, updated_at FROM sites WHERE id = ?'
      )
      .bind(id)
      .first<Site>();
    return result;
  ...
  async createSite(site: Site): Promise<Site> {
    const result = await this.db
      .prepare(
        `
      INSERT INTO sites (group_id, name, url, icon, description, notes, order_num, hidden) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?) 
      RETURNING id, group_id, name, url, icon, description, notes, order_num, hidden, created_at, updated_at
    `
      )
      .bind(
        site.group_id,
  ...
  async updateSite(id: number, site: Partial<Site>): Promise<Site | null> {
    ...
    if (site.hidden !== undefined) {
      updates.push('hidden = ?');
      params.push(site.hidden ? 1 : 0);
    }

    // 构建安全的参数化查询
    const query = `UPDATE sites SET ${updates.join(
      ', '
    )} WHERE id = ? RETURNING id, group_id, name, url, icon, description, notes, order_num, hidden, created_at, updated_at`;
    params.push(id);
    ...
    try {
      ...
      for (const group of data.groups) {
        ...
        if (existingGroup) {
          ...
          // 合并时更新隐藏状态以保持一致
          await this.updateGroup(existingGroup.id as number, {
            hidden: group.hidden ?? (existingGroup.hidden ?? false),
          });
      ...
      for (const site of data.sites) {
        ...
        if (existingSite) {
          ...
          await this.updateSite(existingSite.id as number, {
            ...
            notes: site.notes,
            hidden: site.hidden ?? (existingSite.hidden ?? false),
            // 不更新order_num以保持现有排序
          });
  ...
  async getGroupByName(name: string): Promise<Group | null> {
    const result = await this.db
      .prepare('SELECT id, name, order_num, hidden, created_at, updated_at FROM groups WHERE name = ?')
      .bind(name)
      .first<Group>();
    return result;
  }
  ...
  async getSiteByGroupIdAndUrl(groupId: number, url: string): Promise<Site | null> {
    const result = await this.db
      .prepare(
        'SELECT id, group_id, name, url, icon, description, notes, order_num, hidden, created_at, updated_at FROM sites WHERE group_id = ? AND url = ?'
      )
      .bind(groupId, url)
      .first<Site>();
    return result;
