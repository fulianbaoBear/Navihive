    ...
    try {
      ...
      for (const importGroup of data.groups) {
        ...
        if (existingGroupIndex >= 0) {
          // 已存在同名分组，更新必要字段（包括 hidden）并添加到映射
          mockGroups[existingGroupIndex] = {
            ...mockGroups[existingGroupIndex],
            hidden: importGroup.hidden ?? mockGroups[existingGroupIndex].hidden ?? false,
            updated_at: new Date().toISOString(),
          };
          if (importGroup.id) {
          ...
          const newGroup = {
            ...importGroup,
            id: newId,
            hidden: importGroup.hidden ?? false,
            created_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
          };
      ...
      for (const importSite of data.sites) {
        ...
        if (existingSiteIndex >= 0) {
          ...
          mockSites[existingSiteIndex] = {
            ...
            hidden: importSite.hidden ?? mockSites[existingSiteIndex].hidden ?? false,
            updated_at: new Date().toISOString(),
          };
          stats.sites.updated++;
        } else {
          ...
          const newSite = {
            ...
            hidden: importSite.hidden ?? false,
            created_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
          };
